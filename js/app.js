...
angular.module('daisy', ['ngRoute', 'angular-carousel']).filter('unsafe', function($sce) {
  return function(val) {
    return $sce.trustAsHtml(val);
  };
}).run(function($route, $rootScope, $location, helper) {
  var original;
  ...
  return obj;
}).controller('LightsCtrl', function($window, $location, $routeParams, $anchorScroll, $timeout, $interval, helper) {
  var light, name, stop1, vm, _i, _len, _ref;
  vm = this;
  vm.q = {};
  vm.related = [];
  if (!vm.lights) {
    vm.lights = $window.lights;
  }
  vm.cts = {
    par: 'LED PAR',
    city: 'City Color',
  ...
  vm.init = function() {
    ...
    return stop = $interval(function() {
      if (helper.lights) {
        ...
        stop = null;
        return vm.lights = helper.lights;
      }
    }, 250);
  };
  vm.category = function(category) {
    vm.open = false;
    vm.q = {
      c: category
    };
    return vm.c = vm.cts[category];
  };
  vm.mark = function(mark) {
    vm.closes();
    vm.q = {
      ms: mark
    };
  ...
  vm.view = function(light) {
    ...
    vm.related = vm.lights.slice(pos, pos + 4);
    return $timeout(function() {
      return $location.path("/lights/" + light.n, false);
    }, 200);
  };
  vm.view2 = function(light) {
    vm.on = false;
    $timeout(function() {
      vm.light = '';
      $location.hash('top');
