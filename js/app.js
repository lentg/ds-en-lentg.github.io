...
app.controller('LightsCtrl', function($routeParams, $location, $anchorScroll, $filter, $timeout, $interval, Data) {
  var vm;
  vm = this;
  $anchorScroll();
  vm.active = {};
  ...
  vm.list = function() {
    var run;
    return run = $interval(function() {
      if (Data.lights[0]) {
        vm.lights = Data.lights;
        vm.search = function() {};
        return $interval.cancel(run);
      }
    }, 100);
  };
  ...
  vm.view = function(name) {
    ...
    return run = $interval(function() {
      if (Data.lights[0]) {
        ...
        return angular.forEach(Data.lights, function(light) {
          if (light.n === name) {
            vm.message = {};
            vm.light = light;
            vm.title = light.n;
